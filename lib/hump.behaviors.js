(function($) {
  $(Hump).bind('loaded', function() {
    $('.status').fn({
      display: function() {
        $(this).html(
          $('.passed').length + $('.failed').length + ' test(s), ' + $('.failed').length + ' failure(s), ' + $('.pending').length + ' pending; ' +
          ((new Date() - Hump.suite_start_time)/1000.0).toString() + " seconds elapsed");
      }
    });
    ...
    $('.it').fn({
      ...
      run: function() {
        if (!$(this).data('humpunit.run')) {
          $(this).trigger('pending');
        } else {
          try {
            try {
              $(this).fn('parent').fn('run_befores');
              $(this).data('humpunit.run')();
            } finally {
              $(this).fn('parent').fn('run_afters');
            }
            $(this).trigger('passed');
      ...
      enqueue: function() {
        ...
        $(Hump)
          .queue(function() {
            self.fn('run');
            setTimeout(function() { $(Hump).dequeue() }, 0);
          });
      },
    ...
    $('.before').fn({
      run: function() { $(this).data('humpunit.run')() }
    }); 
    ...
    $(to_run)
      ...
      .fn('enqueue');
    $(Hump).queue(function() { $(Hump).trigger('after') });
  })
})(jQuery);
